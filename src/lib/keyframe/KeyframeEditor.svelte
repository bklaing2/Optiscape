<script lang="ts">
  ...
  $: canAdd = !!keyframe.source && ((keyframe.category === 'sfx' && !!keyframe.start) || (!!keyframe.start && !!keyframe.end))
  $: addButtonText =
    !keyframe.category ? 'Choose audio type' :
    !keyframe.source ? 'Choose/Upload audio' :
    keyframe.category !== 'sfx' && !keyframe.end ? 'Select end range' :
    'Add'
...
<form method="POST" enctype="multipart/form-data" use:enhance={OnSubmit}>
  <Modal bind:show>
    <h2 slot="header" class="text-white text-center text-2xl col-span-full drop-shadow">Add {keyframe.category}</h2>
    ...
    <div class="grid grid-rows-[1fr_min-content] col-span-full">
      ...
      <div class="flex flex-col">
        {#each searchResults.sounds as sound}
          <div class="contents">
            <input type="radio" name="source" value={sound.url} id={sound.url} bind:group={keyframe.source} class="peer hidden" />
            <label for={sound.url} class="flex gap-4 px-8 py-4 border border-transparent rounded-full cursor-pointer hover:bg-orange-200/50 peer-checked:bg-orange-200 peer-checked:border-amber-900/20">
              <Audio src={sound.url} />
              {sound.name}
            </label>
          </div>  
      ...
      <span class="text-xs text-slate-700 truncate">{keyframe.start_snippet || ''}{keyframe.end_snippet || ''}</span>

      <span class="text-xs mt-2">CFI range</span>
      <span class="text-xs text-slate-700">{keyframe.start || ''}{keyframe.end ? ' - ' : ''}{keyframe.end || ''}</span>
      ...
      <input name="percentageEnd" value={keyframe.end_percentage} type="hidden" />
		...
		<button slot="left button" class="py-4 text-rose-700" on:click={Reset} type="button">Discard</button>
    <button slot="right button" class="py-4 disabled:text-slate-600" disabled={!canAdd}>{addButtonText}</button>
  </Modal>
</form>